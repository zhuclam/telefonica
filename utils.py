...
class days_utils:
    ...
    def yesterday(per_day_data):
        yesterday_data = list(
            filter(lambda x: x["date"] == (today() - timedelta(days=1)).strftime("%d/%m/%Y"), per_day_data)
        )
    ...
    def last_month(per_day_data):
        ...
        days_list = [
            (start_of_last_month + timedelta(days=x)).strftime("%d/%m/%Y") for x in range(0, last_month_max_date)
        ]
        data_list = list(filter(lambda x: x["date"] in days_list, per_day_data))
        return functools.reduce(lambda a, x: a + (x["total_calls"]), data_list, 0)
