...
class phone_service:
    ...
    def send_to_end_of_queue(phone, postponed_days=0):
        ...
        if (
            "contestador" in phone.comments
            or "mensaj" in phone.comments
            or "msj" in phone.comments
            or "casilla" in phone.comments
        ):
            phone.comments = ""
    ...
    def handle_unanswered(id, comments, is_test, restore):
        ...
        if restore and restore["last_status"] is not None:
            history_service.restore_call(
                id=id,
                new_status=PHONE_STATUS.unanswered,
                is_test=is_test,
                genuine=is_genuine,
                last_status=restore["last_status"],
            )
        else:
    ...
    def handle_answered(id, comments, is_test, restore):
        ...
        if restore and restore["last_status"] is not None:
            history_service.restore_call(
                id=id,
                new_status=PHONE_STATUS.answered,
                is_test=is_test,
                genuine=is_genuine,
                last_status=restore["last_status"],
            )
    ...
    def handle_non_existent(id, comments, is_test, restore):
        ...
        if restore and restore["last_status"] is not None:
            history_service.restore_call(
                id=id,
                new_status=PHONE_STATUS.non_existent,
                is_test=is_test,
                genuine=is_genuine,
                last_status=restore["last_status"],
            )
    ...
    def handle_no_call(id, comments, is_test, restore):
        ...
        if restore and restore["last_status"] is not None:
            history_service.restore_call(
                id=id,
                new_status=PHONE_STATUS.no_call,
                is_test=is_test,
                genuine=is_genuine,
                last_status=restore["last_status"],
            )
    ...
    def handle_answering_machine(id, comments, is_test, restore):
        ...
        if restore and restore["last_status"] is not None:
            history_service.restore_call(
                id=id,
                new_status=PHONE_STATUS.answering_machine,
                is_test=is_test,
                genuine=is_genuine,
                last_status=restore["last_status"],
            )
        else:
            history_service.register_call(
                id=id, status=PHONE_STATUS.answering_machine, is_test=is_test, genuine=is_genuine
            )
    ...
    def handle_postponed(id, comments, is_test, restore):
        ...
        if restore and restore["last_status"] is not None:
            history_service.restore_call(
                id=id,
                new_status=PHONE_STATUS.postponed,
                is_test=is_test,
                genuine=is_genuine,
                last_status=restore["last_status"],
            )
    ...
    def handle_ignored(id, comments, is_test, restore):
        ...
        if restore and restore["last_status"] is not None:
            history_service.restore_call(
                id=id,
                new_status=PHONE_STATUS.ignored,
                is_test=is_test,
                genuine=is_genuine,
                last_status=restore["last_status"],
            )
    ...
    def handle_rushed(id, comments, is_test, restore):
        ...
        if restore and restore["last_status"] is not None:
            history_service.restore_call(
                id=id,
                new_status=PHONE_STATUS.rushed,
                is_test=is_test,
                genuine=is_genuine,
                last_status=restore["last_status"],
            )
    ...
    def validate_new_phones(phones):
        if not isinstance(phones, list) or not len(phones) > 0:
            return False
        for phone in phones:
...
class task_service:
    def check_task_executed():
        ...
        if status.last_checked == today().date():
            return
...
class mail_service:
    def send_mail(*, to, subject, html):
        message = Mail(from_email="zhuclam@gmail.com", to_emails=to, subject=subject, html_content=html)
